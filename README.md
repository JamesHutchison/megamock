...
with mock.patch("some.hard.to.remember.and.long.dot.path.ClassICareAbout", class_mock) as patch:
    do_something()
```
...
```python
# this import was generated by the IDE with a few keystrokes
from some.hard.to.remember.and.long.dot.path import ClassICareAbout
...
patch.megainstance.method_call.return_value = "some value"
...
`AsyncMegaMock` replaces `AsyncMock`.
...
# use this, which has a static type of MyClass:
